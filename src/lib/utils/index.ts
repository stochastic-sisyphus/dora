...
/**
 * Resolves a potentially relative URL to an absolute URL using the WEBUI_BASE_URL.
 * In the Tauri desktop app, relative URLs resolve to tauri://localhost instead of
 * the remote server, so we need to prepend the base URL for server-relative paths.
 */
export function resolveResourceUrl(url: string): string {
	if (!url || url === '') return '';
	if (url.startsWith('http://') || url.startsWith('https://')) return url;
	if (url.startsWith('data:')) return url;
	if (url.startsWith('/')) return `${get(WEBUI_BASE_URL)}${url}`;
	return url;
}
